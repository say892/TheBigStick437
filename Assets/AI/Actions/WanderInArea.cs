...
[RAINAction]
public class WanderInArea : RAINAction
{
	/// <summary>
	...
	public Expression AreaCenter = new Expression();

	public Expression WanderRadius = new Expression();
	...
	public override ActionResult Execute(AI ai)
	{
		...
		float radius;
		if (WanderRadius.IsValid) radius = WanderRadius.Evaluate<float>(ai.DeltaTime, ai.WorkingMemory);
		else throw new Exception();

		Vector3 center;
		if (AreaCenter.IsValid) center = AreaCenter.Evaluate<Vector3>(ai.DeltaTime, ai.WorkingMemory);
		else throw new Exception();
		Debug.Log(center);

		if (Vector3.Distance(center, tDirection) > radius) {
